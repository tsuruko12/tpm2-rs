...
use crate::{
    ...
    hierarchy::Hierarchy,
    types::{
        PolicyData, SymmetricKeyBits,
        public::{BlockCipher, CipherMode},
        tpm::{
            Tpm2bName, Tpm2bPrivate, Tpm2bPublic, TpmMarshal, TpmUnmarshal, TpmiDhPersistent,
            TpmtPublic,
        },
    },
};
...
CREATE TABLE tpm_keys (
    ...
    policy          BLOB,
    parent_key_name TEXT REFERENCES tpm_keys(key_name),
...
CREATE TABLE symmetric_keys (
    ...
    wrapping_key_id INTEGER UNIQUE REFERENCES wrapping_keys(id)
);
...
const CREATE_WRAPPING_KEY_CLEANUP_TRIGGER: &str = r#"
CREATE TRIGGER IF NOT EXISTS delete_dedicated_wrapping_key
AFTER DELETE ON symmetric_keys
WHEN OLD.wrapping_key_id IS NOT NULL
BEGIN
    DELETE FROM wrapping_keys
    WHERE id = OLD.wrapping_key_id
      AND NOT EXISTS (
          SELECT 1
          FROM symmetric_keys
          WHERE wrapping_key_id = OLD.wrapping_key_id
      );
END;
"#;
...
pub(crate) struct DeleteTarget {
    ...
    pub(crate) persistent_handle: Option<TpmiDhPersistent>,
    pub(crate) obj_name: Tpm2bName,
}
...
impl MetadataStore {
    ...
    pub(crate) fn init(&mut self, key_meta: &[InternalKeyMeta]) -> Result<()> {
        ...
        tx.execute_batch(CREATE_WRAPPING_KEY_CLEANUP_TRIGGER)?;
    ...
    fn load_tpm_key(&self, key_name: &str) -> Result<KeyMeta> {
        let stmt = r#"
            SELECT
                user_keys.key_name,
                user_keys.kind,
        ...
        let persistent_handle = persistent_handle
            ...
            .transpose()?;
        let tpm_key_meta =
            Self::load_tpm_key_meta(public, private, object_name, policy.as_deref())?;
    ...
    fn load_symmetric_key(&self, key_name: &str) -> Result<KeyMeta> {
        let stmt = r#"
            SELECT
                user_keys.key_name,
                user_keys.kind,
    ...
    pub(crate) fn collect_persistent_key(
        ...
        for row in rows {
            let (name, handle, obj_name) = row.map_err(Error::corrupted_store_with_source)?;
            child_names.push(name.clone());

            let persistent_handle = handle
                .map(TpmiDhPersistent::try_from)
                .transpose()
                .map_err(Error::corrupted_store_with_source)?;
            targets.push(DeleteTarget { 
        ...
        for name in child_names {
            self.collect_persistent_key(&name, targets)?;
        }
...
fn save_dedicated_wrapping_key(tx: &Transaction<'_>, key_meta: &TpmKeyMeta) -> Result<i64> {
    ...
    tx.execute(
        r#"
            INSERT INTO wrapping_keys (public, private, object_name, policy)
            VALUES (?1, ?2, ?3, ?4)
        "#,
        params![
            public,
