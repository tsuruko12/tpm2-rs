...
use super::{
    Context, 
    session::{
        CpHashData, ResponseAuthContext, 
        split_prepared_sessions, update_command_hmacs, verify_response_hmac
    }
};
use super::super::{
    ...
    types::{TpmaSession, TpmiDhContext, TpmiShPolicy, TpmRc}
};
...
impl Context {
    ...
    fn flush_context(&mut self, handle: impl Into<TpmiDhContext>) -> Result<()> {
        let command = Command::new(TpmCc::FLUSH_CONTEXT)
            .with_handles([TpmHandle::from(handle.into())]);
